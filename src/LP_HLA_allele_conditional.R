...
sqrt(sum(hla[ind.controls, 'DQB1*05:01']>1.5) / nrow(hla[ind.controls,]))

(sum(hla[ind.controls, 'DQB1*05:01']>0.5) / nrow(hla[ind.controls,]))


# out$Adjusted <- adjustment.chain[i]
# out$Signif <- ifelse(out$FDR_BY<0.05, 'FDR <0.05', 'not significant')
# out %>% filter(Pheno=='NonOral') %>% distinct(ID, .keep_all=T) %>%  
#   # head(20) %>% dplyr::select(c(ID, A1FREQ, BETA, SE, P, Pheno)) %>% kable(format='simple') 
#   head(20) %>% dplyr::select(c(ID, A1FREQ, OR, OR_95CI, P, Pheno)) %>% kable(format='simple') 
# 
# map_dfr(1:length(res), function(i) {
#   out <- map2(res[[i]], c('All', 'NonOral', 'Oral'), function(d, p) process_result(d, p)) %>% 
#     do.call(rbind, .) %>% 
#     arrange(P)
#   out$ID <- gsub(':', '', out$ID, fixed=T)
...
#   out$Signif <- ifelse(out$FDR_BY<0.05, 'FDR <0.05', 'not significant')
#   out %>% return()
# }) %>% fwrite(., 'results/LP_DQ/LP_HLA_summary_stats_09May2024.tsv', sep='\t')
...
tmp <- res.alleles %>% 
  #' filter(!(Adjusted %in% c('DQA1*0101', 'DQA1*0105', 'DQA1*0101 & DQA1*0105', 
  #'                          #'DQA1*0105 & DQB1*0501 &\nDQB1*0602 & B*1302', 
  #'                          'DQA1*0105 & DQB1*0501 &\nDQB1*0602 & B*3501',
  #'                          'DQA1*0101 & DQB1*0501', 'DQB1*0501 domdev', 'DQB1*0501'))) %>% 
...
###
p.res.alleles.2 <- res.alleles.2 %>% filter(!(ID %in% c('DQA1*0105'))) %>% 
  ggplot(aes(OR, ID, xmin = OR_LOWER, xmax = OR_UPPER, color = Pheno %>% factor, shape = Signif)) +
  geom_pointrange(position=position_dodge(width=0.6), size=0.6, linewidth=0.7) +
  xlab('OR') + ylab('') +
  scale_color_manual(values=c("#440154FF", "#21908CFF",  "#FFBF00") %>% rev) + # , #FFD580 ff9248
  scale_shape_manual(name=NULL, values=c(15, 0)) +
  ...
  theme(axis.text.x=element_text(size=10, angle=0, hjust=0.5),
        ...
        panel.border=element_rect(fill=NA, linewidth=0.2))
sapply(seq(1, unique(res.alleles$ID) %>% length, by=2), function(i) {
  p.res.alleles.2 <<- p.res.alleles.2 +
    annotate(geom='rect', ymin=i-0.5, ymax=i+0.5, xmin= 0, xmax=Inf, 
             fill='grey25', alpha=0.05, color=NA) 
}) 
x11(); p.res.alleles.2

jpeg('results/LP_DQ/Alleles2_adj.jpg', width=14, height=4.8, units='in', res=600)
p.res.alleles.2
dev.off()


p.res.alleles.3 <- res.alleles.3 %>% 
  ggplot(aes(BETA, ID, xmin=LOWER, xmax=UPPER, color=Pheno, shape=Signif)) +
  geom_pointrange(position=position_dodge(width=0.6), size=0.6, linewidth=0.7) +
  xlab('beta') + ylab('') +
  scale_color_manual(values=c("#440154FF", "#21908CFF", "#FFBF00") %>% rev) + # #FFD580 ff9248
  scale_shape_manual(name=NULL, values=c(15, 0)) +
  #coord_cartesian(xlim=c(-0.4, 0.69)) +
  geom_vline(xintercept=0, linewidth=0.3, color='black', linetype='dashed') +
  guides(color = guide_legend(override.aes=list(shape = 15))) +
  facet_wrap(vars(Adjusted), scales='free_x', ncol=4) +
  theme_minimal() +
...
sapply(seq(1, unique(res.alleles.3$ID) %>% length, by=2), function(i) {
  p.res.alleles.3 <<- p.res.alleles.3 +
    annotate(geom='rect', ymin=i-0.5, ymax=i+0.5, xmin= -Inf, xmax=Inf, 
             fill='grey30', alpha=0.05, color=NA) 
}) 
x11(); p.res.alleles.3

jpeg('results/LP_DQ/Alleles3_adj.jpg', width=11, height=4.8, units='in', res=600)
p.res.alleles.3
dev.off()



# p.res.alleles <- res.alleles %>% 
#   ggplot(aes(BETA, ID, xmin=LOWER, xmax=UPPER, color=Pheno, shape=Signif)) +
#   geom_pointrange(position=position_dodge(width=0.6), size=0.6, linewidth=0.7) +
#   xlab('beta') + ylab('') +
#   scale_color_manual(values=c("#440154FF", "#21908CFF", "#FFBF00") %>% rev) + # #FFD580 ff9248
...
#   geom_vline(xintercept=0, linewidth=0.3, color='black', linetype='dashed') +
#   guides(color = guide_legend(override.aes=list(shape = 15))) +
#   facet_wrap(vars(Adjusted), scales='fixed') +
#   coord_flip() +
#   theme_minimal() +
...
target.alleles <- c('A*03:01', 'B*13:02', 'B*35:01', 'DQA1*01:02', 'B*08:01', 'DQB1*03:01',
                    'DQB1*02:01', 'DQB1*05:01', 'DQB1*02:02', 'DQB1*06:02')

res <- list(list.files('results/LP_DQ', 'LP_HLA_step2_A0301_[A|O|N]+', full.names = T),
            list.files('results/LP_DQ', 'LP_HLA_step2_DQB10602_[A|O|N]+', full.names = T))

adjustment.chain <- c('A*0301', 'DQB1*0602')
...
res.alleles$Adjusted <- factor(res.alleles$Adjusted, levels=adjustment.chain)#[c(1, 3, 4, 6)])
# arrange alleles by beta
res.alleles$ID <- factor(res.alleles$ID, levels=res.alleles %>% #filter(Adjusted==adjustment.chain[1]) %>%  
                           group_by(ID) %>% summarise(MB=mean(BETA)) %>% arrange(MB) %>% .$ID)
# arrange phenos by beta
res.alleles$Pheno <- factor(res.alleles$Pheno, levels=c('Oral', 'All', 'NonOral'))

# 
res.alleles.A_DQ <- ggplot(res.alleles, # %>% filter(ID %in% target.alleles), 
       aes(BETA, ID, xmin=LOWER, xmax=UPPER, color=Pheno %>% factor, shape=Signif)) +
  geom_pointrange(position=position_dodge(width=0.6), size=0.6, linewidth=0.7) +
  xlab('beta') + ylab('') +
  scale_color_manual(values=c("#440154FF", "#21908CFF",  "#FFBF00") %>% rev) + # , #FFD580 ff9248
  # scale_color_manual(values=c("#2D718EFF", "#FFBF00") %>% rev) + # ,  
  scale_shape_manual(name=NULL, values=c(15, 0)) +
  coord_cartesian(xlim=c(-0.4, 0.69)) +
  geom_vline(xintercept=0, linewidth=0.3, color='black', linetype='dashed') +
  guides(color = guide_legend(override.aes=list(shape = 15))) +
  facet_wrap(vars(Adjusted), scales='fixed', ncol=4, nrow = 2) +
  theme_minimal() +
  theme(axis.text.x=element_text(size=10, angle=0, hjust=0.5),
        axis.text.y=element_text(size=10),
        panel.grid=element_blank(),
        ...
        panel.border=element_rect(fill=NA, linewidth=0.2))
sapply(seq(1, unique(res.alleles$ID) %>% length, by=2), function(i) {
  res.alleles.A_DQ <<- res.alleles.A_DQ +
    annotate(geom='rect', ymin=i-0.5, ymax=i+0.5, xmin= -Inf, xmax=Inf, 
             fill='grey25', alpha=0.05, color=NA) 
}) 
x11(); res.alleles.A_DQ


process_result('results/LP_DQ', 'LP_HLA_step2_DQB10602_[A|O|N]+', full.names=T)


##############

plotAlleleAdj <- function(dd) {
  tmp <- map2(dd, c('All', 'NonOral', 'Oral'), function(d, p) process_result(d, p)) %>% 
    do.call(rbind, .)
  tmp <- tmp$Gene %>% unique %>% map_dfr(function(x) {
    res <- filter(tmp, Gene==x)
    res$Allele <- ''
    res$Allele[which.min(res$P)] <- res$ID %>% str_split_fixed(., '\\*', 2) %>% 
      .[, 2] %>% gsub(':', '', ., fixed=T)
    res %>% return()
  })
  tmp$Gene  <- factor(tmp$Gene, 
                      levels=c('A', 'C', 'B', 'DRB1', 'DQA1', 'DQB1', 'DPB1'))

  tmp %>% ggplot(aes(Gene, -log10(P), color=Pheno, label=Allele)) +
    geom_point(shape='\u2015', size=7.5) +
    scale_color_manual(values=c("#440154FF", "#21908CFF", "#FFBF00")) +
    geom_text(nudge_x=.5, size=2.5) +
    coord_cartesian(clip='off', ylim=c(0, 210)) +
    xlab('') +
    theme_minimal() +
    theme(axis.text.x=element_text(size=10),
          ...
          legend.title=element_blank(),
          plot.margin=unit(c(.1,.9,.1,.5), 'cm'))
}

ggarrange(plotlist=map(res[1:2], plotAlleleAdj), nrow=2)

jpeg('results/LP_DQ/Alleles.jpg', width=5, height=6, units='in', res=600)
ggarrange(plotlist=map(res[1:2], plotAlleleAdj), nrow=2)
dev.off()
